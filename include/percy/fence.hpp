...
namespace percy
{
    class fence
    {
        ...
        public:
			...
			void set_nr_node(int nr_nodes1)
			{
				_nr_nodes = nr_nodes1;
			}
			void set_nr_level(int nr_level1)
			{
				_nr_levels = nr_level1;
			}
    ...
    ***************************************************************************/
    class rec_fence_generator
    {
        private:
			...
			std::function<void(rec_fence_generator*)> _callback1;
        ...
        public:
			...
			void
				find_fences(moodycamel::ConcurrentQueue<fence>& q, fence& fence_local)
			{
				assert(_initialized);
				_callback1 = [&q](rec_fence_generator* gen) {
					const auto nr_levels = gen->nr_levels();
					fence f(gen->nr_nodes(), nr_levels);
					for (int i = 0; i < nr_levels; i++) {
						f[i] = gen->nodes_on_level(nr_levels - 1 - i);
    ...
    ***************************************************************************/
    ...
    inline void
        generate_fences_abc(spec& spec, moodycamel::ConcurrentQueue<fence>& q, unsigned nr_level_max)
    {
        rec_fence_generator gen;

        for (int l = 1; l <= spec.nr_steps && l <= nr_level_max; l++) {
            gen.reset(spec.nr_steps, l, spec.get_nr_out(), spec.fanin);
            gen.generate_fences(q);
        }
    }
	...
	inline void
		find_fences(spec& spec, moodycamel::ConcurrentQueue<fence>& q)
	{
		rec_fence_generator gen;
	...
	inline void
		find_new_fences(fence& fence_local, moodycamel::ConcurrentQueue<fence>& q)
	{
		rec_fence_generator gen;
		gen.find_fences(q, fence_local);
		for (int l = 0; l < fence_local.nr_levels(); l++) {
			//gen.reset(fence_local.nr_nodes()+1, fence_local.nr_levels(), spec.get_nr_out(), spec.fanin);
			fence f_new = fence_local;
			int nr_level = fence_local.nr_levels();
			f_new.set_nr_node(fence_local.nr_nodes() + 1);
			f_new[l]++; // add flexible new node
			while (!q.try_enqueue(f_new)) {
				std::this_thread::yield();
			}
		}
	}
