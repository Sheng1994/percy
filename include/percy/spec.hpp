...
namespace percy
{
    ...
    class spec
    {
        ...
        public:
            ...
            int verbosity = 1; ///< Verbosity level for debugging purposes
			...
			int nr_dags; /// Number of DAGs to be synthesized
			std::vector<int> output_dags; ///< Position of outputs for a DAG
            int nr_out; /// the number of function to be synthesized (triv + nontriv)
            int out_width; /// the number of steps (included in a reversed order) that can be choosed to be output
            ...
            void set_primitive(Primitive primitive)
            {
				...
				if ((newop == 1) & (fanin == 3)) {
					...
					tt = inputs[0] | inputs[1] | inputs[2];
					compiled_primitives.push_back(tt);
					tt = ternary_majority(inputs[0], inputs[1], inputs[2]);
					compiled_primitives.push_back(tt);
					...
					case SPIN:
						...
						break;
					case SPIN2:
						tt = inputs[0] & inputs[1]; // AND
						compiled_primitives.push_back(tt);
						tt = ~(inputs[0] & inputs[1]); // NAND
						compiled_primitives.push_back(tt);
						tt = inputs[0] | inputs[1]; // OR
						compiled_primitives.push_back(tt);
						tt = ~(inputs[0] | inputs[1]); // NOR
						compiled_primitives.push_back(tt);
						tt = inputs[0] ^ inputs[1]; // XOR
						compiled_primitives.push_back(tt);
						tt = ~(inputs[0] ^ inputs[1]); // XNOR
						compiled_primitives.push_back(tt);
						tt = inputs[0]; // X
						compiled_primitives.push_back(tt);
						tt = inputs[1]; // Y
						...
						break;
					case CUST:
						std::vector<std::string> tts_all = read_tts("primitives.txt");
						int k = 0, nr_tts;
                        nr_tts = std::stoi(tts_all[0]);
						for (auto ip = 0; ip < nr_tts; ip++) {
                            int nr_ttin;
                            nr_ttin = std::stoi(tts_all[nr_tts + ip+1]);
							kitty::dynamic_truth_table tt(nr_ttin);
							kitty::create_from_hex_string(tt, tts_all[1+k++]);
							compiled_primitives.push_back(tt);
						}
					}
				}
            ...
            void set_primitive_3in()
            {
                int fanin3 = 3;
                compiled_primitives.clear();
                kitty::dynamic_truth_table tt(fanin3);
                std::vector<kitty::dynamic_truth_table> inputs;
                for (int i = 0; i < fanin3; i++) {
                    inputs.push_back(kitty::create<kitty::dynamic_truth_table>(fanin3));
                    kitty::create_nth_var(inputs[i], i);
                }
                /*tt = inputs[0];
                tt._bits[0] = 0;
                for (int i = 0; i < pow(2, pow(2, fanin3))-1; i++) {
                    tt._bits[0]++;
                    compiled_primitives.push_back(tt);
                }*/

                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & (~inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & (inputs[0] | ~inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & (inputs[0] & inputs[1]));
                ...
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & (inputs[0] ^ inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & inputs[0]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & ~inputs[0]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (inputs[0] | inputs[1])) | (~inputs[2] & ~inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & (~inputs[0] | inputs[1])) | (~inputs[2] & (inputs[0] | inputs[1]));
                ...
                tt = (inputs[2] & (inputs[0] ^ inputs[1])) | (~inputs[2] & ~inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & (inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & (~inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & (inputs[0] | ~inputs[1]));
                ...
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & (inputs[0] ^ inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & ~inputs[0]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[0]) | (~inputs[2] & ~inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & ~inputs[0]) | (~inputs[2] & (inputs[0] | inputs[1]));
                ...
                tt = (inputs[2] & ~inputs[0]) | (~inputs[2] & ~inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & (inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & (~inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & (inputs[0] | ~inputs[1]));
                ...
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & (inputs[0] ^ inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & inputs[0]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & ~inputs[0]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & inputs[1]) | (~inputs[2] & ~inputs[1]);
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & ~inputs[1]) | (~inputs[2] & (inputs[0] | inputs[1]));
                compiled_primitives.push_back(tt);
                tt = (inputs[2] & ~inputs[1]) | (~inputs[2] & (~inputs[0] | inputs[1]));
                ...
                tt = inputs[2] & ~inputs[1] | ~inputs[2] & inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & (~inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & (inputs[1] | ~inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & (inputs[1] & inputs[2]);
                ...
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & (inputs[1] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & ~inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (inputs[1] | inputs[2]) | ~inputs[0] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & (~inputs[1] | inputs[2]) | ~inputs[0] & (inputs[1] | inputs[2]);
                ...
                tt = inputs[0] & (inputs[1] ^ inputs[2]) | ~inputs[0] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & (inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & (~inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & (inputs[1] | ~inputs[2]);
                ...
                tt = inputs[0] & inputs[1] | ~inputs[0] & (inputs[1] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & ~inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[1] | ~inputs[0] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & ~inputs[1] | ~inputs[0] & (inputs[1] | inputs[2]);
                ...
                tt = inputs[0] & ~inputs[1] | ~inputs[0] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & (inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & (~inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & (inputs[1] | ~inputs[2]);
                ...
                tt = inputs[0] & inputs[2] | ~inputs[0] & (inputs[1] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & ~inputs[1];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & inputs[2] | ~inputs[0] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[0] & ~inputs[2] | ~inputs[0] & (inputs[1] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[0] & ~inputs[2] | ~inputs[0] & (~inputs[1] | inputs[2]);
                ...
                tt = inputs[0] & ~inputs[2] | ~inputs[0] & inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & (~inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & (inputs[0] | ~inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & (inputs[0] & inputs[2]);
                ...
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & (inputs[0] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & inputs[0];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & ~inputs[0];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (inputs[0] | inputs[2]) | ~inputs[1] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & (~inputs[0] | inputs[2]) | ~inputs[1] & (inputs[0] | inputs[2]);
                ...
                tt = inputs[1] & (inputs[0] ^ inputs[2]) | ~inputs[1] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & (inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & (~inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & (inputs[0] | ~inputs[2]);
                ...
                tt = inputs[1] & inputs[0] | ~inputs[1] & (inputs[0] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & ~inputs[0];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[0] | ~inputs[1] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & ~inputs[0] | ~inputs[1] & (inputs[0] | inputs[2]);
                ...
                tt = inputs[1] & ~inputs[0] | ~inputs[1] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & (inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & (~inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & (inputs[0] | ~inputs[2]);
                ...
                tt = inputs[1] & inputs[2] | ~inputs[1] & (inputs[0] ^ inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & inputs[0];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & ~inputs[0];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & inputs[2] | ~inputs[1] & ~inputs[2];
                compiled_primitives.push_back(tt);
                tt = inputs[1] & ~inputs[2] | ~inputs[1] & (inputs[0] | inputs[2]);
                compiled_primitives.push_back(tt);
                tt = inputs[1] & ~inputs[2] | ~inputs[1] & (~inputs[0] | inputs[2]);
