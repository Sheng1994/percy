...
namespace percy
{
    ...
    private:
        ...
        int get_op_var(const spec& spec, int step_idx, int var_idx) const
        {
            ...
            return ops_offset + step_idx * nr_op_vars_per_step + var_idx;
        }
            ...
            const int l) const
        {
            ...
            int offset = 0;
            for (int ip = 0; ip < i; ip++) { // traverse all steps
                const auto n = spec.nr_in + ip; // ID of the ip-th step
                offset += (n * (n - 1) * (n - 2)) / 6; // possible input combinations
            }

            int svar_ctr = 0;
            for (int lp = 2; lp < spec.nr_in + i; lp++) { // highest ID of input
                for (int kp = 1; kp < lp; kp++) { // middle ID of input
                    for (int jp = 0; jp < kp; jp++) { // lowest ID of input
                        if (l == lp && k == kp && j == jp) {
    ...
    public:
        encoder_3in(solver_wrapper& solver)
        {
        ...
        void create_variables(const spec& spec)
        {
            ...
            sim_offset = nr_sel_vars + nr_op_vars + out_offset;
            total_nr_vars = nr_sel_vars + nr_op_vars + out_offset + nr_sim_vars;

            if (spec.verbosity) {
                printf("Creating variables (3IN)\n");
                printf("nr steps = %d\n", spec.nr_steps);
        ...
        bool create_fanin_clauses(const spec& spec)
        {
            ...
            for (int i = 0; i < spec.nr_steps; i++) {
                auto ctr = 0;
                for (int l = 2; l < spec.nr_in + i; l++) { // highest ID of input
                    for (int k = 1; k < l; k++) { // middle ID of input
                        for (int j = 0; j < k; j++) { // lowest ID of input
        ...
        */
        ...
        bool add_simulation_clause_op(
            const spec& spec,
            const int t, // output bit
            const int i, // current step
            const int j, // the first input
            const int k, // the second input
            const int l, // the third input
            const int a, // the bit of the first input
            const int b, // the bit of the second input
            const int c, // the bit of the third input
            const int d, // the output bit of the current operator
            const int sel_var  // step variable that determine the input sources
            )
        {
            //int pLits[5];
            int ctr = 0;
            ...
            if (l < spec.nr_in) {
                ...
                pLits[ctr++] = pabc::Abc_Var2Lit(
                    get_sim_var(spec, l - spec.nr_in, t), d);
            }
            ...
            if (b | c | d) { // inputs should not be all 0s
                pLits[ctr++] = pabc::Abc_Var2Lit(
                    get_op_var(spec, i, ((d << 2) | (c << 1) | b) - 1), 1 - a);
            }
                ...
                for (int l = 2; l < spec.nr_in + i; l++) {
                    for (int k = 1; k < l; k++) {
                        for (int j = 0; j < k; j++) {
                            ...
                            //ret &= add_simulation_clause(spec, t, i, j, k, l, 0, 0, 0, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 0, 0, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 0, 1, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 0, 1, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 1, 0, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 1, 0, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 1, 1, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 0, 1, 1, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 0, 0, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 0, 0, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 0, 1, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 0, 1, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 1, 0, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 1, 0, 1, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 1, 1, 0, sel_var);
                            ret &= add_simulation_clause_op(spec, t, i, j, k, l, 1, 1, 1, 1, sel_var);
                        }
                    }
                }
                assert(ret);
        ...
        bool encode_3in(const spec& spec)
        {
            assert(spec.nr_in >= 3);

            create_variables(spec); // build the list of variables
            create_main_clauses(spec); // truth table operations and output clauses
        ...
        bool fence_create_output_clauses(const spec& spec)
        {
            auto status = true;
        ...
        encode(const spec& spec, const fence& f)
        {
            ...
            if (!fence_create_output_clauses(spec)) {
                return false;
            }
        ...
        void extract_chain_3in(const spec& spec, chain_3in& chain)
        {
        ...
        void fence_extract_3in(const spec& spec, chain_3in& chain)
        {
            int op_inputs[3] = { 0, 0, 0 };
