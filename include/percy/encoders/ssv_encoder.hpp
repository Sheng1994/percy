        ...
        public:
            ...
            *******************************************************************/
            ...
            bool create_tt_clauses(const spec& spec, const int t)
            {
                ...
                for (int i = 0; i < spec.nr_steps; i++) {
                    ...
                    for (int j = 0; j < nr_svars_for_i; j++) {
                        ...
                        const auto& fanins = svar_map[svar]; // choose which pi/step as input
                        ...
                        while (true) {
                            next_assignment(fanin_asgn); // 10 01 11, 00
            ...
            bool add_simulation_clause(
                    const spec& spec, 
                    const int t, // truth table
                    const int i, // step
                    const int svar, // step variable
                    const int output, // 0 or 1
                    const int opvar_idx, // 
                    ...
                    const std::vector<int>& fanin_asgn)
            {
                ...
                for (int j = 0; j < spec.fanin; j++) {
                    ...
                    if (child < spec.get_nr_in()) {
						auto t_test = t + 1;
						auto child_test = 1 << child;
						auto result_test = t_test & child_test;
                        if ((((t + 1) & (1 << child) ) ? 1 : 0 ) != assign) { // t - odd or even; child - *2
                            return true;
                        }
                    } else {
                ...
                pabc::Vec_IntSetEntry(vLits, ctr++,
                        pabc::Abc_Var2Lit(get_sel_var(svar), 1)); // inputs
                pabc::Vec_IntSetEntry(vLits, ctr++,
                        pabc::Abc_Var2Lit(get_sim_var(spec, i, t), output)); // the output of step i for truth tablet bit t
